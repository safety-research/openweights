...
def create_authenticated_client(supabase_url: str, supabase_anon_key: str, auth_token: Optional[str] = None, api_key: Optional[str] = None):
    ...
    headers = {}
    if api_key:
        if not api_key.startswith('ow_'):
            raise ValueError("API key must start with 'ow_'")
...
class OpenWeights:
    def __init__(self, 
        ...
        1. Session token: Use auth_token from Supabase authentication
        2. API key: Use a long-lived API key starting with 'ow_'
        
        One of auth_token or api_key must be provided.
        """
        self.supabase_url = supabase_url or os.getenv('SUPABASE_URL')
        self.supabase_key = supabase_key or os.getenv('SUPABASE_ANON_KEY')
        self.auth_token = auth_token or os.getenv('SUPABASE_AUTH_TOKEN')
        self.api_key = api_key or os.getenv('OPENWEIGHTS_API_KEY')
